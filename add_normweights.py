...
def add_normweight(conf):
    ...
    for dsid in dsids:
        ...
        my_df_dsid = my_df_dsid.Define("normweight", "mcEventWeight * pileupWeight * {} * {} * {} / {}".format(
            pmg_info['xs'], pmg_info['eff'], pmg_info['kfactor'], sum_of_weights[dsid]))
        if debug:
            hist = my_df_dsid.Histo1D("normweight")
            output_file = TFile(
                'normweight_histogram_dsid_{}.root'.format(dsid), 'RECREATE')
            hist.Write()
            output_file.Close()

        # Write new DFs as TTrees to ROOT files (unless requested not to)
        if not dry_run:
            out_full_file_name = '{}/{}'.format(output_dir, os.path.basename(
                file_name).replace(".root", "_expanded.root"))
            log.info('Taking a snapshot of the dataframe as a ROOT file: {}'.format(
                out_full_file_name))
...
def expand_ttrees(args):
    ...
    logging.basicConfig(level='INFO' if not debug else 'DEBUG',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inDir', action='store',
                        dest='input_dir', default='', help='Path to input files')
    parser.add_argument('--outDir', action='store', dest='output_dir',
                        default='', help='Directory where output/updated files will be located')
    parser.add_argument('--pmgFileName', action='store', dest='pmg_file_name', default='/cvmfs/atlas.cern.ch/repo/sw/database/GroupData/dev/PMGTools/PMGxsecDB_mc16.txt',
                        help='Full file name containing PMG cross sections, efficiencies, etc')
    parser.add_argument('--ttreeName', action='store', dest='ttree_name',
                        default='trees_SRRPV_', help='Name of input TTrees')
    parser.add_argument("--dryRun",  action='store_true', dest='dry_run',
                        default=False, help='do a dry run, without actually doing anything')
    parser.add_argument('--debug', action='store_true',
                        dest='debug', default=False, help='Debug flag')
    parser.add_argument(
        "--ncpu", help="Number of cores to use for multiprocessing. If not provided multiprocessing not done.", default=1, type=int)
